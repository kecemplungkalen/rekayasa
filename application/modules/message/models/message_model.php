...
//where lbn.name='inbox' group by ab.id_address_book;
	...
	function get_by_thread($thread=false,$perpage=0,$start=0,$where=false,$keyword=false)
	{
		if($thread)
		{
			$this->db->select('max(inbox.id_inbox) as id_inbox,max(recive_date) as recive_date,min(read_status) as read_status,thread,is_delete,count(id_inbox) as total, inbox.number,first_name,last_name,max(send_status) as send_status');
			$this->db->join('address_book','address_book.id_address_book=inbox.id_address_book','left');			
